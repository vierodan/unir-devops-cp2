...
# Source of script with sensitive data about azure credentials. 
#(This .sh is not included on the git repository because contains sensitive information)
...
#Run the Ansible playbook - Deploy Voting App (Backend and Frontend) into AKS
...
# Run the Ansible playbook - Run nginx container on Azure VM using Podman
